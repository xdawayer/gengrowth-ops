...
**图片 — 已完成并 live（2026-06-09）：**
- 8 篇全部配图：7 个色页**上下文驱动 hero**（如 red=鲜红 vs 浊红对立、blue=浅蓝→靛蓝光谱，表达该页核心，非通用装饰）+ pillar 顶部 hero + **内联「7 色×脉轮」对照信息图**（带文字标注，放在"7 aura colors"章节内，正是本清单原要求）。
- hero 同时渲染进静态 stub（爬虫/图片索引可见 + alt），全 8 页 HTTP 200、h1=1 无回归。
- 生成走 Gemini（你登录的 Google 会话，非 API key）；建了**可复用配图系统**（plan 驱动脚本 + 渲染器内联图支持），后续文章配图复用同一流程，非 aura 专用。
