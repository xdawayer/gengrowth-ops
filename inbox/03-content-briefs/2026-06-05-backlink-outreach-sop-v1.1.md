...
**原理（Ahrefs 官方确认）：**
> 一个外链传递的 DR 权重 ≈ 来源站 DR ÷ 其链出的唯一域名数

因此，DR 高但链出过于分散的站点，单条链接实际权重极低。用以下公式计算**均分DR**：

```
均分DR = DR ÷ Dofollow Linked Domains（链出唯一域名数）
```
...
**分层通过标准（基于22条真实成交外链推导，2026-06）：**

| DR 范围 | 最低均分DR | 最大链出域名数 | 注意 |
|--------|----------|------------|------|
| DR 0–30 | ≥ 0.06 | ≤ 300 | 低DR须高集中度才有传递价值 |
| DR 30–50 | ≥ 0.05 | ≤ 600 | 性价比最高的目标区间 |
| DR 50–70 | ≥ 0.03 | ≤ 1,500 | 对数刻度补偿，可容忍更大分散度 |
| DR 70+ | ≥ 0.02 | ≤ 4,000 | 极高权威站，即使稀释仍有价值 |

**真实排除案例：**
- businesstomark.com：DR 62，链出 16,709 域名，均分DR 0.004 → 排除（链接农场特征）
- feast-magazine.co.uk：DR 72，链出 6,749 域名，均分DR 0.011 → 排除
- englishlush.com：DR 70，链出 4,507 域名，均分DR 0.016 → 排除

**真实通过案例：**
- appkod.com：DR 71，链出 3,067 域名，均分DR 0.023 → 通过（DR 70+，≥ 0.02）
- greatercollinwood.net：DR 53，链出 612 域名，均分DR 0.087 → 通过
- techbles.com：DR 44，链出 470 域名，均分DR 0.094 → 通过

**快速判断公式（录入 Airtable / Excel，B=DR，C=Linked Domains，D=均分DR）：**

```excel
=IF(B2>=70, IF(C2<=4000, IF(D2>=0.02,"通过","均分不足"),"LD超限"),
  IF(B2>=50, IF(C2<=1500, IF(D2>=0.03,"通过","均分不足"),"LD超限"),
  IF(B2>=30, IF(C2<=600,  IF(D2>=0.05,"通过","均分不足"),"LD超限"),
             IF(C2<=300,  IF(D2>=0.06,"通过","均分不足"),"LD超限"))))
```
