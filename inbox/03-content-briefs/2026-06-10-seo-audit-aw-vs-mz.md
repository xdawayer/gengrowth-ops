...
优先级：saturn (737词) → sun (867词) → north-node (968词) → natal-chart (971词)  
目标：每篇扩展到1,500–2,000词，补充FAQ模块  

**② 为41篇无E-E-A-T信号的黄道/基础词页面添加作者+日期**

这些页面（libra/aries/virgo/venus等）是搜索量最高的页面，却完全没有author/datePublished  
操作：在JSON-LD schema中补充 `author` + `datePublished` + `dateModified` 字段  
...
chakra-system-overview, how-to-read-birth-chart, june/july-2026-planetary-transits, four-element-framework  

**④ 每篇文章图片数从avg 2提升至avg 5–6**  
重点方向：占星盘示意图（可程序化生成）、星座对应配色信息图、时间轴/周期图表  

**⑤ 修复36%的At-a-glance缺口**  
53篇缺少At-a-glance的文章，优先补充字数>1,500词的高流量页面  
