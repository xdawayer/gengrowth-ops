...
2. **重写问题片段** - 用自然的替代方案替换 AI 痕迹，不删减原文覆盖的信息点
3. **保留含义** - 保持核心信息完整
4. **维持语调** - 匹配预期的语气（正式、随意、技术等）
5. **按场景注入灵魂** - 观点、博客、个人表达可以加入个性；技术、法律、HR、SOP、参考文档应保持朴素准确

---

## 声音校准（可选）

如果用户提供了作者本人过往写作样本，先分析样本再改写：

- 句长习惯：短句多、长句多，还是长短混合
- 用词层级：口语、专业、学术，还是介于两者之间
- 段落开头：直接进入判断，还是先交代背景
- 标点习惯：是否常用括号、分号、破折号、短句
- 转场方式：显性连接词多，还是直接进入下一点
- 个人表达：是否常用第一人称、直接判断、犹豫或自我修正

改写时不要只“去 AI 味”，还要用样本里的真实表达习惯替换 AI 结构。没有样本时，默认使用自然、直接、有节奏变化的中文。
...
6. **保留专业判断** - 可以使用经济学、管理学、科技行业的通俗专业名词，但必须服务于具体判断，不能当作高级感装饰
...
- **博客、观点稿、创始人表达、社交媒体内容**：可以保留第一人称、情绪、判断和一点锋芒。
- **产品文档、技术文档、SOP、HR、合同、法务、事实型调研**：以准确、清楚、可复现为准，不主动加入观点、幽默或个人化表达。
- **营销文案**：可以更有力度，但要用具体利益、场景和证据，不用口号、金句和空泛形容词撑气势。

专业名词可以使用，但要满足三条：

1. **通俗且常见**：如机会成本、转换成本、规模效应、网络效应、边际成本、飞轮、技术债、CAC、LTV、PMF、J 曲线。
2. **能解释当前判断**：名词后面要落到具体机制、行为或结果。
3. **不伪装权威**：不能用“经济学上说”“管理学认为”替代证据。

**不好的写法：**
> 从管理学视角看，这本质上是组织飞轮的系统性重构。

**更好的写法：**
> 这里更像一个飞轮：内容越多，SEO 入口越多；入口越多，带来的用户问题越多；这些问题又会反过来补充新内容。

## 个性与灵魂

避免 AI 模式只是工作的一半。无菌、没有声音的写作和机器生成的内容一样明显。好的写作背后有一个真实的人。

只有在内容和作者声音需要时才应用本节。百科、技术、法律、HR、SOP、参考文档里，中立朴素就是正确的人类声音，不要硬加观点或第一人称。
...
**需要注意的词汇：** 资料有限、公开信息较少、保持低调、鲜少公开个人信息、可能成长于、据推测、可以认为、基于现有资料推断

**问题：** AI 在没有资料时，常写一段“资料不足”的废话，再补上听起来合理但没有来源的猜测。人物介绍尤其常见。

**改写前：**
> 关于她早年经历的公开资料较少，这表明她一直保持低调，可能成长于一个重视教育的家庭，这也影响了她后来对教育改革的兴趣。
...
**问题：** 文档或注释写得像在讲“这次改了什么”，而不是描述当前系统是什么。除非是 changelog、release note、迁移指南，否则应写当前状态。

**改写前：**
> 这个函数被新增用于替代原来遍历全部项目的方案，旧方案会导致 O(n²) 性能问题。

**改写后：**
> 这个函数使用哈希表做 O(1) 查询，避免逐项遍历带来的 O(n²) 成本。

---

### 21. 金句公式

**需要注意的词汇：** X 是 Y 的语言、X 是 Y 的货币、X 是 Y 的架构、X 不是工具而是镜子、X 成为陷阱、底层逻辑、系统性重构

**问题：** AI 喜欢把普通判断包装成可引用的句子。它们听起来有气势，但通常没有增加精度。

**改写前：**
> 对称是信任的语言。效率在团队忘记人的时候成为陷阱。

**改写后：**
> 对称布局通常让用户更容易预测界面行为。团队如果只优化流程速度，可能会忽略真实使用场景。
...
**需要注意的词汇：** 真正的问题是、本质上、归根结底、核心在于、从根本上说、更深层的问题、问题的关键、真正重要的是

**问题：** 这些表达假装正在穿透表象，但后面往往只是普通判断。能直接说具体问题时，不要先摆姿态。

**改写前：**
> 真正的问题是团队能否适应。归根结底，核心在于组织是否具备转型能力。

**改写后：**
> 问题是团队能否改变现有习惯。现在的阻力主要来自销售和交付流程没有统一。

---

### 26. 开场标识语

**需要注意的词汇：** 下面我们来拆解、让我们深入探讨、接下来我们看看、话不多说、你需要知道的是、先说结论、我们先从……开始

**问题：** AI 常先宣布“我要开始讲了”，而不是直接讲。这会让内容像教程脚本。

**改写前：**
> 下面我们来拆解 Next.js 的缓存机制。你需要知道的是，它有多个层级。

**改写后：**
> Next.js 有多层缓存，包括请求记忆、数据缓存和路由缓存。
...
- ✓ **使用了"此外""然而"等连接词？** 考虑删除
- ✓ **三段式列举？** 改为两项或四项
- ✓ **出现专业名词？** 确认它解释了具体机制，而不是装饰
- ✓ **出现"本质上/核心在于/真正的问题"？** 能否直接说具体问题
- ✓ **文档在讲"新增/替代/优化"？** 如果不是 changelog，改成当前状态

---

## 误判保护

不要把所有干净、正式或结构清楚的文字都判成 AI。以下情况不能单独作为 AI 痕迹：
...
判断时看**模式组合**，不要看单个词。一个"本质上"不说明什么；"本质上"加金句、三段式、模糊归因、通用积极结论，才说明文本需要清理。
