...
test('生成自有短链：使用 astrologywiki 主站短路径，并输出映射关系', () => {
  ...
  assert.equal(core.toDisplayShortUrl(shortLink), '/go/x9k2p7');
});
...
test('GenGrowth 站点工具：独立目录、标题和脚本可用', () => {
  ...
  assert.equal(gengrowth.html.includes('valueForAction'), true);
...
test('GenGrowth 站点工具：只允许 gengrowth.ai 域名并生成长短链', () => {
  ...
  assert.equal(gengrowth.core.toDisplayShortUrl('https://www.gengrowth.ai/go/g9n2th'), '/go/g9n2th');
