...
function createGenGrowthKeywordSheet() {
  ...
  try {
    SpreadsheetApp.getUi().alert(
      '✅ 模板创建成功！\n\n' +
      '使用步骤：\n' +
      '1. 在「关键词主表」的 A–H 列输入数据\n' +
      '   （I/J/K 列公式自动计算，无需手动填写）\n' +
      '2. 各桶视图（趋势词/快速胜利/战略词/长尾词）自动更新\n' +
      '3. 内容发布后在「📊内容追踪」填写实际排名和流量\n' +
      '4. Day 30 / Day 60 复盘时更新「📈来源分析」\n\n' +
      '💡 提示：Trends比值留空默认为平稳，策略覆盖勾选后强制进快速胜利桶'
    );
  } catch(e) {
    // 非交互环境时忽略弹窗
  }
