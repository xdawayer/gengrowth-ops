---
title: GenGrowth 内部增长系统 MVP PRD（集群完整性修订版）
date: 2026-06-04
updated: 2026-06-04
type: prd
version: v0.8
status: final
supersedes: 2026-05-15-gengrowth-internal-growth-mvp-prd-v0.7.md
tags:
  - gengrowth
  - prd
  - growth-system
  ...
  - astrologywiki
aliases:
  - GenGrowth MVP PRD v0.8
  - 内部增长系统 MVP 集群完整性修订版
  - astrologywiki 增长 MVP v0.8
---

# GenGrowth 内部增长系统 MVP PRD v0.8（集群完整性修订版 · 当前执行基准）

## 0. 本版定位

本版是 v0.7 的迭代，作为当前唯一执行基准，**取代 v0.7**。

v0.8 只修三个影响落地协同的点：关键词分桶与集群架构解耦、内容 SOP 从 v2.0 对齐到 v2.3、`keyword-sheet-setup.gs` 从 v3.1 升到 v3.2。v0.7 中已确认的 MVP 范围、6-ID、量产线/精修线、地区闸门、CTA 策略继续继承。

### 0.1 v0.8 的 5 处修订

| # | 问题 | 处理 | 章节 |
|---|---|---|---|
| 1 | `❌跳过` 同时表达“无关词”和“竞争暂缓”，导致高 DR 核心词从集群架构中消失 | 拆成 `竞争建议` 与 `生产状态`：DR/KD 只建议可做/暂缓；负向词才判无关 | §7.1 §7.3 附录 D |
| 2 | DR/KD 被当作架构删除规则 | 分桶继续存在，但只表示机会类型；集群必需词即使高 DR/KD 也保留在集群架构 | §7.3 |
| 3 | 选题登记表被误用成关键词筛选源 | 明确选题登记表是页面生产视图；关键词主表是事实源；新增生产候选视图 | §6.1 §7.5 附录 C/D |
| 4 | PRD 内容 SOP 仍引用 v2.0，与 v2.3 冲突 | 内容 SOP 改为 `2026-05-20-seo-pipeline-sop-v2.3.md`；同步 T1/T2/T3 时长、Direct Answer Block、按需 FAQ、Day 30 科学排查 | §4.3 §7.5 §7.9 §11 |
| 5 | 两份评审意见缺少正式回应 | 在评审文档末尾补“采纳意见与改动说明” | §16 |
...
| `2026-05-20-seo-pipeline-sop-v2.3.md`（v2.3 运营极速落地版）| **现行内容 SOP** | 作为内容生产 SOP，本版与之对齐（§7.5、附录 C）|
| `2026-05-14-seo-pipeline-sop-v2.md` / `SEO内容生产流水线_v0.18 / v0.19` | superseded | 不再使用，v2.3 取代 |
...
### 4.3 内容生产 SOP：采纳 v2.3（修订 1）

决策：

- **v2.3 定为现行内容生产 SOP**，v2.0 / v0.18 / v0.19 标记 superseded，不再使用。
- v2.3 继承了集群版生产卡、Pillar/Spoke/Standalone 角色、1+N 折叠，并补上运营极速落地要求：T1 45-60 分钟、T2 10-15 分钟、T3 <5 分钟、Direct Answer Block、按需 FAQ、独立 5 秒 QA、Day 30 科学排查。
- 选题登记表采用 **v2.3 页面生产视图**：只放已经决定生产或已生产的 page。关键词筛选、暂缓、集群必需等状态回到关键词主表 V 列，不在选题登记表里解决。
...
| 5 内容生产引擎 | 集群级内容生产（量产线 / 精修线，SOP = v2.3）| MVP 主模块 |
...
| Page Asset | 选题登记表 sheet（v2.3 页面生产视图）| 实际页面 / 文章 / 工具页 |
...
### 7.1 Step 0：表格级作战契约 = 升级 keyword-sheet-setup.gs 到 v3.2（修订 4）

v0.8 给出具体执行：**把 `docs/03-marketing/03-seo/keyword-sheet-setup.gs` 升级到 v3.2**，一次生成一个工作簿、包含全部 6-ID 体系的表，并修正“DR过滤=架构删除”的错误口径。
...
#### 7.3.0 分桶、竞争建议、生产状态的关系（v0.8 新增）

关键词主表保留四桶分级，但分桶只回答“这是什么机会”，不再回答“是否从架构中删除”。

| 层 | Google Sheet 字段 | 作用 | 规则 |
|---|---|---|---|
| 竞争建议 | N 列 `竞争建议` | 提醒当前站是否适合正面竞争 | DR 差值 > 30 → `⏸暂缓`；≤30 → `✅可做`。这不是过滤关卡。 |
| 机会分桶 | O/R 列 `分桶_自动` / `分桶` | 标注机会类型 | 趋势词 / 快速胜利 / 战略词 / 长尾词 / 无关。高 DR/KD 词仍可归入某个桶。 |
| 生产状态 | V 列 `生产状态` | 决定是否进入生产候选和选题登记表 | `可生产` / `暂缓` / `集群必需` / `无关` / `已建卡` / `已发布`。 |

简化原则：

- **无关词**：命中 NEGATIVE_KEYWORDS 或人工判定无关，O/R 列为 `❌无关`，V 列为 `无关`。
- **高 DR/KD 但相关词**：不再 `❌跳过`；照常归入四桶，N 列提示 `⏸暂缓`，V 列默认 `暂缓`。
- **集群核心词**：即使 N 列为 `⏸暂缓`，只要是 Pillar、核心实体或系列骨架词，V 列标 `集群必需`，进入集群架构和生产候选。
- **选题登记表**：只接收 V 列为 `可生产` / `集群必需` / `已建卡` / `已发布` 的页面，不接收 `暂缓` 或 `无关`。
...
1. **`.gs` 加 `NEGATIVE_KEYWORDS` 配置区 + O 列前置否决**：关键词包含 `miami / dade / bus tracker / hub city / trimet` 等任意负向词 → 直接 `❌无关`。最快、一劳永逸（规格见附录 D）。
2. **（可选）K 列子串匹配改词边界匹配**，减少假阳性。
3. **种子词纪律**：禁止用单个多义词做种子，写进关键词 SOP 的执行注意。
4. **集群生成那一步**：cluster doc 是 LLM 从关键词列表自动生成的，必须读取 `keyword_id/关键词 + R列分桶 + N列竞争建议 + V列生产状态`。不能只喂快速胜利/长尾词，否则会漏掉高 DR 但集群必需的核心词。
...
内容生产 SOP 采用 `inbox/03-content-briefs/2026-05-20-seo-pipeline-sop-v2.3.md`（v2.3）。本节只定义 v2.3 与本 PRD 的对接，不重复 v2.3 的六步流水线细节。
...
选题登记表是**页面生产视图**，不是关键词筛选表。它只放已经决定要生产、已建卡或已发布的 page；暂缓词和无关词停留在关键词主表，不进入选题登记表。

v2.3 继续保留 `page_id`、`cluster_id`、显式 `page_role`、`content_angle`、`psych_safety_flag`、`journal_prompts`。行位置布局只作视觉辅助，公式和内链不依赖行位置。

进入选题登记表的来源：关键词主表 V 列为 `可生产` / `集群必需` / `已建卡` / `已发布` 的词，经集群归并后建成页面卡。月搜索量/KD 继续 VLOOKUP 主表；一页可关联多个关键词，Associated Keywords 承接 1+N 折叠。

#### 7.5.3 Tier 规则与审核产能模型

| Tier | 用途 | 人工要求 | 审核工时 |
|---|---|---|---|
| T1 重装 | Pillar / 战略页 / 心理风险页（多为精修线）| 人工 SERP + Reddit 搜证 + 审稿 + psych safety QA | 45–60 分钟 |
| T2 标准 | Series 主力页 | AI 起草 + 人工检查 Direct Answer / H2 逻辑 / 内链 / CTA | 10–15 分钟 |
| T3 占位 | 极长尾支撑页（多为量产线 aura/Vedic）| AI 组装 + 5 秒红线 QA | <5 分钟 |

**审核产能模型（瓶颈约束）**：T3 压缩到 <5 分钟后，节省的产能反哺 T1。推论：**量产线必须以 T3/T2 为主，T1 每周 ≤ 3 篇**。Pillar 通常是 T1；高 DR 但集群必需的核心词可先做 T3/T2 占位或进入 Backlog，但不能从集群架构中删除。

#### 7.5.4 验收

每篇内容开头用自然的 Direct Answer Block 直接回答搜索意图，严禁机械套 `## TL;DR`；FAQ 按需生成，不对 T3 强制；每篇归属一个集群；每篇有内链和 CTA；T1 内容有真实 Friction 或 SERP 差异证据；精修线 healing 页面有 journal prompts；发布后写回 URL 和复盘日期。
...
| `keyword-sheet-setup.gs` 升 v3.2 | 一键生成单一工作簿 6-ID + 生产候选视图（规格见附录 D）| Day 1–3 |
| 关键词主表 N/O/R/V 列口径更新 | N=竞争建议；O/R=机会分桶；V=生产状态 | Day 1–2 |
| 集群级 Brief / 选题登记表 v2.3 / 周度行动清单 模板 | Markdown / Sheet 模板 | Day 2–3 |
...
PGB（并入种子模板 Day-0）1 份；P0 集群规划 2–3 个（含 1 量产 + 1 精修）；Backlog 集群 8–12 个；页面发布或待发布 15–20 篇；选题登记表 v2.3 每个发布页都有；GA4 + 事件已上线；Newsletter + ESP 已上线（Day 5–7）；周度行动清单 1 份。
...
→ 集群级 Brief → 选题登记表 v2.3
→ 内容生产（SOP = v2.3 + 附录 A 页面模板 + 附录 B 心理安全规则）
...
| 文档 | 唯一职责 | 状态 |
|---|---|---|
| 本 PRD v0.8 | MVP 范围、9 步流程、内容双线、数据对象、验收 | 现行执行基准 |
...
| `2026-05-20-seo-pipeline-sop-v2.3.md`（v2.3）| 单页面内容装配零件、极速 QA、部署与复盘规则 | **现行内容 SOP** |
| `keyword-sheet-setup.gs` | 一键生成工作簿（升 v3.2）| 已更新 |
| `SEO内容生产流水线 v0.18 / v0.19` | —— | **superseded** |
| 附录 A 页面模板 / 附录 B 心理安全规则 | 页面结构骨架 / 语言边界 | 现行 |

### 11.3 待办交付物

- ✅ `keyword-sheet-setup.gs` 升 v3.2（2026-06-04 完成）：N 列竞争建议、O/R 列机会分桶、V 列生产状态、生产候选视图、选题登记表 v2.3 页面生产视图。
- ✅ 选题登记表 v2.3（已并入 .gs v3.2 生成）。
- ✅ `keyword-research-sop.md` 补"种子词不可用单个多义词"（v2.4 已加）。
- ✅ 内容 SOP 对齐 v2.3：T1/T2/T3 时长、Direct Answer Block、按需 FAQ、Day 30 科学排查。
...
- `keyword-sheet-setup.gs` 升 v3.2，生成含主题集群表、选题登记表 v2.3、CTA Map、结果复盘表、生产候选视图的单一工作簿。
- 关键词主表 N/O/R/V 列口径已更新：竞争建议 / 机会分桶 / 生产状态。
- 完成 PGB（并入种子模板 Day-0）。
- 完成 6-ID 体系与 CTA Map。
...
这套流程**框架层可复用**（9 步流程、6-ID、集群中心、关键词 SOP、v2.3 内容 SOP、种子实验模板、刷新规则、`.gs` 生成器），**实例层不可复用**（astrologywiki 的量产/精修划分、psych-safety、healing 角度、具体集群）。
...
| M1.5 | `.gs` v3.2 一键生成工作簿；选题登记表 v2.3；SOP 稳定；新产品 onboard = 跑 `.gs` + 填 ⚙️配置 + 填 PGB/Day-0 |
| M2 | GenGrowth 自动起草 PGB、自动聚类、API 拉指标、GSC+GA4 基础归因 |
| M3 | Agent 周循环编排、自动外链 prospect、CRM 外联、实验系统 |

可复用引擎 = `keyword-sheet-setup.gs` + SOP 三件套（关键词 SOP / v2.3 内容 SOP / 种子模板）。每个新站重复：跑脚本 → 填配置 → 关键词研究 → 聚类 → 生产。仍需人工：相关性 / 地区判断、psych-safety 边界、集群角度决策。
...
6. 内容生产 SOP = v2.3；v2.0 / v0.18 / v0.19 superseded。选题登记表升 v2.3 页面生产视图。
7. 6-ID 体系统一进一个 Google 工作簿；`keyword-sheet-setup.gs` 升 v3.2 生成。
...
| **v0.8** | **2026-06-04** | **final** | **响应 2026-05-20 PRD 诊断与 2026-05-21 关键词工作流风险评审：内容 SOP 从 v2.0 升至 v2.3；`keyword-sheet-setup.gs` 升 v3.2；关键词主表拆分 N列竞争建议 / O-R列机会分桶 / V列生产状态；DR/KD 不再作为集群架构删除规则；选题登记表明确为页面生产视图；新增生产候选视图；Day 30 改为科学排查链路；两份评审文档末尾补采纳与改动说明。取代 v0.7。** |
...
## 附录 C：选题登记表 v2.3 列结构（页面生产视图）

选题登记表不是关键词筛选表。关键词是否无关、暂缓、集群必需，统一在关键词主表 V 列处理；选题登记表只承载已经决定生产或已经生产的页面。

| 列 | 字段 | 来源 | 填写时机 | 说明 |
|---|---|---|---|---|
| 1 | Target Keyword | v2.3 | 建卡 | =primary_keyword |
| 2 | Associated Keywords | v2.3 | 建卡 | =secondary_keywords，1+N，上限 7 |
| 3 | 月搜索量 | v2.3 | 自动 | VLOOKUP 关键词主表 |
| 4 | KD | v2.3 | 自动 | VLOOKUP 关键词主表 |
| 5 | Intent | v2.3 | 建卡 | Info/Compare/Tutorial/Utility/Experience |
| 6 | Tier | v2.3 | 建卡 | T1/T2/T3 |
| 7 | Template | v2.3 | 建卡 | 指附录 A 页面模板 |
| 8 | Entity | v2.3 | Brief | 主权实体 |
| 9 | Friction | v2.3 | Tier 闸门，仅 T1/T2 | —— |
| 10 | Logic | v2.3 | Tier 闸门，仅 T1/T2 | —— |
| 11 | CTA | v2.3 | 发布后 | 可由 page_role 经 CTA Map 推导 |
| 12 | GSC Keywords | v2.3 | 维护期 | 刷新用 |
| 13 | Status | v2.3 | 实时 | 待写/写作中/质检/已发布/已刷新 |
| 14 | URL | v2.3 | 发布后 | —— |
| 15 | Last Audit | v2.3 | 维护期 | —— |
...
## 附录 D：keyword-sheet-setup.gs v3.2 工作簿列结构规格
...
**Sheet 关键词主表（A–X 现有 24 列，N/O/R/V 列口径修订）**：C 列「月搜索量」取**目标国家**数值（Ahrefs 设国家后默认即该国量，零额外工作量），分桶阈值据此判断；**不新增全球量列、不算每词 us_share**（早期太重，详见 §3.3）。

关键列：

| 列 | 字段 | 口径 |
|---|---|---|
| N | 竞争建议 | DR差值 > 30 → `⏸暂缓`；≤30 → `✅可做`。仅建议，不删除架构。 |
| O | 分桶_自动 | 趋势词 / 快速胜利 / 战略词 / 长尾词 / 无关。表示机会类型。 |
| R | 分桶 | 手动覆盖后的最终机会分桶。 |
| V | 生产状态 | `可生产` / `暂缓` / `集群必需` / `无关` / `已建卡` / `已发布`。决定是否进入生产候选和选题登记表。 |

O 列分桶公式最前面加一层负向词否决——
```text
=IF(A2="","",
 IF(SUMPRODUCT(--(ISNUMBER(SEARCH(IFERROR(配置!$A$28:$A$45,""),A2))))>0,"❌无关",
 <原 O 列四桶逻辑> ))
```
含任意负向词 → 直接 `❌无关`。DR/KD 不在 O 列做跳过，不再阻止相关词进入集群架构。
...
**Sheet 生产候选（新增）**：从关键词主表筛选 V 列为 `可生产` / `集群必需` / `已建卡` / `已发布` 的行。运营从这里进入建卡，不直接从四桶视图搬词。

**Sheet 选题登记表 v2.3（新增）**：见附录 C，21 列。月搜索量/KD 用 VLOOKUP 从关键词主表取。只承载 page，不承载暂缓词或无关词。
