...
### 11.3 待办交付物（M1.5，不在本 PRD 内）

- `keyword-sheet-setup.gs` 升 v3.0（规格见附录 D）。
- 选题登记表升 v2.1（规格见附录 C）。
- `keyword-research-sop.md` 补"种子词不可用单个多义词"的执行注意。
- v2.0 内容 SOP 补精修线专属字段说明（content_angle / psych_safety / journal_prompts）。
