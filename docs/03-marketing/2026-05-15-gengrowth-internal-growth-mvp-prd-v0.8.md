...
本文件在 v0.7 原文基础上迭代，当前作为唯一执行基准，**取代 v0.7**。

v0.8 不重写整体框架，只修三类影响协同落地的问题：

1. **关键词分桶与集群架构解耦**：DR/KD 不再作为架构删除规则；高 DR/KD 但集群必需的词保留在集群骨架中。
2. **内容 SOP 对齐 v2.3**：现行内容 SOP 从 v2.0 改为 v2.3，同步 T1/T2/T3 时长、Direct Answer Block、按需 FAQ、Day 30 科学排查。
3. **Google Sheet 口径简化**：关键词主表拆为 `竞争建议` / `机会分桶` / `生产准入` / `生产状态`；选题登记表明确为页面生产视图。
...
| 产能 = AI 批量 + 1 人审核，已达到 10+ 篇/天 | 生成和 T3 机械审核不再是主要瓶颈；瓶颈转为生产准入、集群完整性、发布回填与 P0/T1 质量抽检 |
...
**产能现状：** astrologywiki.com 当前已达到 **AI 批量 + 1 人审核 10+ 篇/天**。生成和 T3 机械审核不再是主要瓶颈；当前瓶颈转为：生产准入是否准确、集群骨架是否完整、发布/URL/page_id 回填是否及时，以及 P0/T1 页面是否保住质量红线。本版据此把字段与 QA 分为“批量生产可自动推进”和“集群/质量必须人工抽检”两层（§7.3、§7.5）。
...
### 4.3 内容生产 SOP：采纳 v2.3（修订 1，v0.8 更新）
...
v0.8 更新：`2026-05-20-seo-pipeline-sop-v2.3.md` 已成为现行内容 SOP。v2.0 / v0.18 / v0.19 标记 superseded，不再作为执行基准。

决策：

- **v2.3 定为现行内容生产 SOP**，v2.0 / v0.18 / v0.19 标记 superseded，不再使用。
- v2.3 继承集群版生产卡、Pillar/Spoke/Standalone 角色、1+N 折叠，并补上运营极速落地要求：T1 45-60 分钟、T2 10-15 分钟、T3 <5 分钟、Direct Answer Block、按需 FAQ、独立 5 秒 QA、Day 30 科学排查。
- 选题登记表采用 **v2.3 页面生产视图**：只放已经决定生产或已生产的 page。关键词筛选、暂缓、集群必需等状态回到关键词主表，不在选题登记表里解决。
...
| 6 分发与外联 | 轻量社媒 / 社区 / 外链机会 | 人工执行；三子渠道分工见 §7.6 + 团队 RACI |
...
关键词主表     A–AB 28 列，N=竞争建议，O/R=机会分桶，V/W/X=生产准入，Y=生产状态
主题集群表     新增（cluster_id / track / us_share / content_angle …）
选题登记表     v2.3 页面生产视图（page_id / cluster_id 外键）
...
#### 7.3.0 分桶、竞争建议、生产准入、生产状态的关系（v0.8 新增）
...
| 生产准入_自动 | V 列 `生产准入_自动` | 自动给出默认生产准入 | `可生产` / `暂缓` / `无关`。 |
| 手动生产准入 | W 列 `手动生产准入` | 人工覆盖自动准入 | `可生产` / `暂缓` / `集群必需` / `无关`。 |
| 生产准入 | X 列 `生产准入` | 决定是否进入生产候选和选题登记表 | W 非空则用 W，否则用 V。`可生产` / `集群必需` 进入候选。 |
| 生产状态 | Y 列 `生产状态` | 只记录生产进度 | `未开始` / `已建卡` / `已发布` / `已合并` / `暂停`。 |

简化原则：

- **无关词**：命中 NEGATIVE_KEYWORDS 或人工判定无关，O/R 列为 `❌无关`，X 列最终为 `无关`。
- **高 DR/KD 但相关词**：不再 `❌跳过`；照常归入四桶，N 列提示 `⏸暂缓`，V 列默认 `暂缓`。
- **集群核心词**：即使 N 列为 `⏸暂缓`，只要是 Pillar、核心实体或系列骨架词，W 列人工标 `集群必需`，X 列最终进入生产候选。
- **选题登记表**：只接收 X 列为 `可生产` / `集群必需` 的页面候选，不接收 `暂缓` 或 `无关`。
...
4. **集群生成那一步**：cluster doc 是 LLM 从关键词列表自动生成的，必须读取 `keyword_id/关键词 + R列分桶 + N列竞争建议 + X列生产准入`。不能只喂快速胜利/长尾词，否则会漏掉高 DR 但集群必需的核心词。
...
内容生产 SOP 采用 `2026-05-20-seo-pipeline-sop-v2.3.md`（v2.3）。本节只定义 v2.3 与本 PRD 的对接，不重复 v2.3 的六步流水线细节。
...
进入选题登记表的来源：关键词主表 X 列为 `可生产` / `集群必需` 的词，经集群归并后建成页面候选，再审核建卡。月搜索量/KD 继续 VLOOKUP 主表；一页可关联多个关键词，Associated Keywords 承接 1+N 折叠。建卡后把 `page_id` 回填到关键词主表 Z 列，并把 Y 列生产状态更新为 `已建卡`。
...
**分工（三条子渠道，Owner 完整 RACI 见 `docs/05-governance/people-ops/team-collaboration/2026-06-16-growth-team-roles-and-division.md`）**：

| 子渠道 | 策略 | 执行 |
|---|---|---|
| 自有社媒分发 + Social Probe / 社区观察 | AI 内容运营（彭满） | AI 内容运营（彭满） |
| 外链机会 | SEO 马博洋（标的页/锚文本/质量门槛） | SEO 马博洋（联系 / PR / 换链内容自己把控）；彭满转交社区里发现的 lead |
| KOL / KOC 外部合作发布 | KOL 运营（**待补**）；带链标的由 SEO 定 | KOL 运营（**待补**）；彭满供素材/卖点/红线 |

> KOL/KOC 为外部合作渠道，第一阶段轻量动作不含，待 KOL 运营到岗后按其 SOP 启动。
...
量产线 · 已在产：Aura Colors 1A
  动作：继续 aura 工厂；给已发布 6 篇补工具页 CTA + 集群内链；按 10+ 篇/天产能推进，本周可生产 50–70 篇，但发布节奏受 P0 集群完整性、收录反馈和 URL 回填质量约束
...
PGB（并入种子模板 Day-0）1 份；P0 集群规划 2–3 个（含 1 量产 + 1 精修）；Backlog 集群 8–12 个；页面发布或待发布 50–70 篇（按 10+ 篇/天能力，实际发布节奏由集群完整性与收录反馈控制）；选题登记表 v2.3 每个发布页都有；GA4 + 事件已上线；Newsletter + ESP 已上线（Day 5–7）；周度行动清单 1 份。
...
- ✅ `keyword-sheet-setup.gs` 升 v3.3（2026-06-05 完成）：生成主题集群表 / 选题登记表 v2.3 / CTA Map / 结果复盘表 / 生产候选视图；N列竞争建议、V/W/X生产准入、Y生产状态。
- ✅ 选题登记表 v2.3（已并入 .gs v3.3 生成）。
...
- `keyword-sheet-setup.gs` 升 v3.3，生成含主题集群表、选题登记表 v2.3、CTA Map、结果复盘表、生产候选视图的单一工作簿。
- 关键词主表 O 列已加负向词否决。
- 完成 PGB（并入种子模板 Day-0）。
- 完成 6-ID 体系与 CTA Map。
...
| Day 60 | 300–500 篇或等效资产（不以页数硬冲，按有效 cluster 和收录反馈调速）；冲刺日 PV 5000；按 cluster + us_share 复盘 | GSC + GA4 |
...
| 时点 | 内容产出 | 数据目标 |
|---|---|---|
| Day 7 | 50–70 篇生产或待发布 | GSC 跑通，URL 已提交；抽查 P0 集群骨架 |
| Day 14 | 100–140 篇累计生产或待发布 | 收录率 ≥ 70%，出现 impressions；根据收录反馈调速 |
| Day 30 | 180–300 篇累计生产或待发布 | ≥ 20 词进 Top 50；找到首批有效 cluster |
| Day 45 | 250–400 篇累计生产或待发布 | 找到 2–3 个有效 cluster，并暂停无信号集群扩产 |
| Day 60 | 300–500 篇累计生产或待发布 | 冲刺日 PV 5000（美国为主）；按有效 cluster 而非单纯页数复盘 |

红灯条件：Day 14 收录率 < 50% → 暂停扩产先修技术；Day 30 美国 impressions < 5000 → 重做 SERP / brief；连续 3 天 page_id/URL 回填滞后 → 暂停新建卡先补数据链路；某周 T1 页面 > 3 篇 → 审核溢出；nakshatra 等 us_share 低集群流量高但美国 PV 无增长 → 该集群不计入目标、把产能调回 aura 与精修线。
...
| M1.5 | `.gs` v3.3 一键生成工作簿；选题登记表 v2.3；SOP 稳定；新产品 onboard = 跑 `.gs` + 填 ⚙️配置 + 填 PGB/Day-0 |
...
6. 内容生产 SOP = v2.3；v2.0 / v0.18 / v0.19 superseded。选题登记表升 v2.3。
...
11. 页面卡按 Tier 分层；当前 10+ 篇/天产能下，瓶颈不是 T3 生成，而是生产准入、集群完整性、发布回填与 T1/P0 质量抽检；T1 每周 ≤ 3 篇。
...
| **v0.8** | **2026-06-05** | **final** | **在 v0.7 原文件基础上迭代：响应 2026-05-20 PRD 诊断与 2026-05-21 关键词工作流风险评审；内容 SOP 从 v2.0 升至 v2.3；`keyword-sheet-setup.gs` 升 v3.3；关键词主表拆分 N列竞争建议 / O-R列机会分桶 / V-W-X列生产准入 / Y列生产状态；DR/KD 不再作为集群架构删除规则；选题登记表明确为页面生产视图；Day 30 改为科学排查链路。取代 v0.7。** |
...
## 附录 D：keyword-sheet-setup.gs v3.3 工作簿列结构规格
...
**Sheet 关键词主表（A–AB 28 列，N/O/R/V/W/X/Y/Z 口径修订）**：C 列「月搜索量」取**目标国家**数值（Ahrefs 设国家后默认即该国量，零额外工作量），分桶阈值据此判断；**不新增全球量列、不算每词 us_share**（早期太重，详见 §3.3）。O 列分桶公式最前面加一层负向词否决；V/W/X/Y/Z 列用于生产准入、生产进度与 page_id 回填。
```text
=IF(A2="","",
...
**Sheet 生产候选（新增）**：按关键词主表 X 列 `生产准入 = 可生产 / 集群必需` 自动筛选，作为从关键词进入页面建卡的中间视图。

**Sheet 选题登记表 v2.3（新增）**：见附录 C，21 列。月搜索量/KD 用 VLOOKUP 从关键词主表取。该表一行一个 page，不是一行一个 keyword。
